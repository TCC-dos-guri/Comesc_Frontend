...
const disabledBtn = computed(() =>
  formInputs.value.some(el => el.value === '') || !selectedState.value
);
...
async function submitForm() {
  ...
  try {
    ...
    setTimeout(() => {
      successMessage.value = '';
      router.push('/employee'); // Lista de endereços
    }, 1500);
  } catch (error) {
    errorMessage.value = error.response?.data?.message || 'Erro ao cadastrar o endereço. Tente novamente.';
  } finally {
    ...
    <div class="flex justify-center p-4 md:p-8">
      <div class="w-full xl:w-full bg-white rounded-2xl shadow-lg p-6 md:p-10">
        ...
        <div class="mb-6 md:mb-8">
          <h2 class="text-2xl md:text-3xl font-bold text-[#261D47] mb-2">Cadastro de Endereço</h2>
          <p class="text-gray-600">Preencha os dados para cadastrar um novo endereço</p>
        </div>
