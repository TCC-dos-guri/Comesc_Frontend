...
async function submitForm() {
  ...
  try {
    ...
    successMessage.value = 'Cor cadastrada com sucesso!';
    formInputs.value.forEach(input => input.value = '');

    // Redirecionar ou resetar a página
    setTimeout(() => {
      successMessage.value = '';
      router.push('/employee'); // exemplo: lista de cores
    }, 1500);
  } catch (error) {
    errorMessage.value = error.response?.data?.message || 'Erro ao cadastrar a cor. Tente novamente.';
  } finally {
    ...
    <div class="flex justify-center p-4 md:p-8">
      <div class="w-full xl:w-full bg-white rounded-2xl shadow-lg p-6 md:p-10">
        ...
        <div class="mb-6 md:mb-8">
          <h2 class="text-2xl md:text-3xl font-bold text-[#261D47] mb-2">Cadastro de Cor</h2>
          <p class="text-gray-600">Preencha os dados para cadastrar uma nova cor</p>
        </div>
